...
import {
  ...
  UpdateTermCommand,
} from "@/api/terms/term.contracts.ts";
...
export function useTerm(callback?: () => Promise<void>) {
  ...
  async function changeVisibility(termId: number, visibility: boolean) {
  ...
  async function duplicateTerm(id: number, taxonomy: string) {
  ...
  async function removeTerm(id: number, taxonomy: string) {
  ...
  async function saveCardSettings(
    ...
    try {
      if (
        ...
        formFields.thumbnailActive?.length ||
        ...
        await termsApi.addImages({
          ...
          thumbnailActive: formFields.thumbnailActive?.[0] ?? null,
  ...
  async function dragTerm(
    element: any,
    params: Omit<UpdateSortIndexCommand, "id" | "sortIndex">,
  ) {
